...
const ReviewWithoutLines = (comment) => {
    const timeago = moment(comment.comment.created).fromNow();

    return <div className="review-component">
    <div className="flex flex-row mt-2">
        <p className="text-lg"><span><FontAwesomeIcon icon={faFullStar} className="full-star"/> {comment.comment.reviewEntity.rating} </span> &nbsp;&nbsp; {comment.comment.comment}</p>
    </div>
    <div className="flex flex-row mb-2">
        ...
        <h1>{comment.comment.userEntity.name}</h1>
...
export default ReviewWithoutLines;